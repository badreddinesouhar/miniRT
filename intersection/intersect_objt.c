...
/*   Updated: 2023/10/02 01:29:31 by aet-tass         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	intersect_sphere(t_shape *sphere, t_ray ray, t_intersections **intersec)
{
	...
	discriminat.delta = (discriminat.b * discriminat.b) - (4 * discriminat.a * discriminat.c);
	if (discriminat.delta < 0)
		return ;
	inter.t1 = ((-discriminat.b - sqrt(discriminat.delta)) / (2 * discriminat.a));
	inter.t2 = ((-discriminat.b + sqrt(discriminat.delta)) / (2 * discriminat.a));
	sort_intersection_list(intersec, ft_lstnew(create_intersection(inter.t1, sphere)));
	if (!compare_float(inter.t1, inter.t2))
		sort_intersection_list(intersec, ft_lstnew(create_intersection(inter.t2, sphere)));
}
